...
inline NEOLIX_STATUS_LIST  getMaxContours(cv::Mat binaryImage, std::vector<std::vector<cv::Point>> &contours,int &maxContourId)
{
...
*/
inline void findMinRectangle(std::vector<cv::Point>(* maxcontour), cv::RotatedRect &rectangle)
{
    ///寻找最小外接矩形
...
NEOLIX_STATUS_LIST calBoundingRectangle(cv::Mat srcImage, std::vector<cv::Point> &contuor, float &PixXLenght, float PixYLength,cv::Point2f point[])
{
    cv::Mat destImage;
    std::vector<cv::Point> maxcomtour;
...
NEOLIX_STATUS_LIST  PopRang(cv::Mat &srcImg,std::vector<cv::Point>& contour,float &PixLength,float &PixWidth,std::vector<cv::Point2f> &point)
{
