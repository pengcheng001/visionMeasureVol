...
namespace neolix
{
	...
	static std::vector<cv::Point2f> points(10);
				...
				IN_  bool  useRect)
{
	...
	if(true == useRect)
    {
		cv::Rect rects = cv::Rect(g_rect_para.at<int>(0, 0), g_rect_para.at<int>(1, 0),
	...
	//height = static_cast<float>(paddis - boxDistance) - REFIX;
