    ...
    while(count < iters)
    {
     ...
     //   std::cout<<"===============find three rands points============="<<<<std::endl<<std::endl;
       ...
       std::cout<<"three rands poins: "<<std::endl;
        ...
        if(cos_val > 0 && std::abs(cos_val - 1.0) < 0.1 )continue;
        if(cos_val < 0 && std::abs(cos_val + 1.0) < 0.1) continue;
        break;
	   ...
	   L_Eq = (cv::Mat_<double>(3, 3) << (double)(((*ptcld)[index[0]])[0]) + left_x,
        ...
        R_Eq = (cv::Mat_<double>(3,1)<<( ((*ptcld)[index[0]])[2]),
                                       ( ((*ptcld)[index[1]])[2]),
                                       ( ((*ptcld)[index[2]])[2]));
	/*	std::cout << (*ptcld)[index[0]]  << std::endl;
		std::cout << (*ptcld)[index[1]]  << std::endl;
		std::cout << (*ptcld)[index[2]]  << std::endl;
		std::cout << L_Eq << std::endl;
    ...
    //   std::cout<<"cal diffDis= "<<diffDis<<std::endl;
     ...
     ///ofile.open("watch.txt");
      totalDepth = 0;
       ...
       {
		  dis_up = Plane.at<double>(0,0)* (((*ptcld)[i])[0] + left_x) +
				   Plane.at<double>(1,0)* (((*ptcld)[i])[1] + left_y) + 
